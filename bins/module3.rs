...
fn main() {
    ...
    let source = gst::ElementFactory::make(
        "uridecodebin", Some ("source")
    ).unwrap();
    let convert = gst::ElementFactory::make(
        "audioconvert", Some("convert")
    ).unwrap();
    let sink = gst::ElementFactory::make(
        "autoaudiosink", Some("sink")
    ).unwrap();
    let resample = gst::ElementFactory::make(
        "audioresample", Some("resample")
    ).unwrap();
